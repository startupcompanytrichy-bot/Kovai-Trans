...
.ef-input:focus { border-color:#d97706; box-shadow:0 0 0 2px rgba(217,119,6,.12); }
...
/* ── Bill upload ── */
.bill-upload-zone { border:2px dashed #fed7aa; border-radius:10px; padding:18px; text-align:center; cursor:pointer; transition:all .2s; background:#fffbeb; }
.bill-upload-zone:hover { border-color:#d97706; background:#fef3c7; }
.bill-upload-zone i { font-size:26px; color:#fed7aa; display:block; margin-bottom:6px; }
.bill-upload-zone .buz-title { font-size:13px; font-weight:600; color:#d97706; }
...
.btn-save-exp { background:linear-gradient(135deg,#d97706,#b45309); color:#fff; border:none; border-radius:8px; padding:10px 26px; font-size:13px; font-weight:700; cursor:pointer; box-shadow:0 4px 14px rgba(217,119,6,.35); display:inline-flex; align-items:center; gap:6px; }
.btn-save-exp:hover:not(:disabled) { box-shadow:0 6px 20px rgba(217,119,6,.45); transform:translateY(-1px); }
...
.ef-select2 .select2-container--default.select2-container--open  .select2-selection--single {
    border-color:#d97706 !important; box-shadow:0 0 0 2px rgba(217,119,6,.12) !important;
}
</style>
    ...
    <div class="row align-items-center">
        <div class="col-md-8" style="position:relative;z-index:1;">
            <div style="display:inline-flex;align-items:center;gap:5px;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.25);border-radius:20px;padding:3px 12px;font-size:11px;font-weight:700;letter-spacing:.5px;margin-bottom:5px;">
                <i class="ti-pencil"></i> Edit Expense
            </div>
            <h4>Edit Expense Entry</h4>
            <div class="sub">Update expense details, amount and bill.</div>
        </div>
...
<form id="expForm" action="{{ route('expense.update', $expense->id) }}" method="POST" enctype="multipart/form-data">
@csrf @method('PUT')
...
<div class="col-lg-8">
    ...
    <div class="ef-card ef-select2">
        <div class="ef-card-header">
            <div class="ch-icon" style="background:#fffbeb;color:#d97706;"><i class="ti-receipt"></i></div>
        ...
        <div class="ef-card-body">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="category" id="categorySelect" class="form-control ef-input select2-exp" required>
                            ...
                            @endforeach
                            <option value="__add_new__" data-icon="ti-plus" data-color="#d97706" data-bg="#fffbeb">＋ Add New Category</option>
                        </select>
                ...
                <div class="col-md-6">
                    <div class="form-group" id="traderGroup">
                        <label class="ef-label" style="display:flex;align-items:center;justify-content:space-between;">
                            <span>Trader <span id="traderCategoryBadge" style="display:none;margin-left:6px;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;background:#fffbeb;color:#d97706;border:1px solid #fed7aa;"></span></span>
                            <button type="button" id="quickAddTraderBtn"
                                style="background:none;border:none;padding:0;font-size:11px;font-weight:700;color:#d97706;cursor:pointer;display:flex;align-items:center;gap:3px;">
                        ...
                        <div id="noTradersMsg" style="display:none;font-size:11px;color:#d97706;margin-top:4px;">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="payment_mode" class="form-control ef-input select2-exp">
                            <option value="cash"   {{ old('payment_mode', $expense->payment_mode ?? 'cash') === 'cash'   ? 'selected' : '' }}>💵 Cash</option>
                            <option value="credit" {{ old('payment_mode', $expense->payment_mode) === 'credit' ? 'selected' : '' }}>💳 Credit</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-4">
                    ...
                    <div class="form-group">
                        ...
                        <select name="trip_id" id="tripSelect" class="form-control ef-input select2-exp @error('trip_id') is-invalid @enderror">
                            ...
                            <option value="{{ $t->id }}"
                                ...
                                {{ old('trip_id', $expense->trip_id) == $t->id ? 'selected' : '' }}>
                                {{ $t->trip_no }}
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="driver_id" id="driverSelect" class="form-control ef-input select2-exp @error('driver_id') is-invalid @enderror">
                            ...
                            <option value="{{ $d->id }}" {{ old('driver_id', $expense->driver_id) == $d->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea name="notes" rows="3" class="form-control ef-input"
                    placeholder="Describe the expense...">{{ old('notes', $expense->notes) }}</textarea>
            </div>
...
<div class="col-lg-4">
    ...
    <div class="ef-card">
        ...
        <div class="ef-card-body">
            ...
            <div class="mb-3">
                <div style="font-size:11px;font-weight:700;color:#8a94a6;margin-bottom:6px;text-transform:uppercase;letter-spacing:.3px;">Current Bill</div>
                @php $ext = pathinfo($expense->bill_image, PATHINFO_EXTENSION); @endphp
                @if(in_array(strtolower($ext), ['jpg','jpeg','png']))
                    <img src="{{ asset('storage/'.$expense->bill_image) }}" style="width:100%;border-radius:8px;border:2px solid #c6f6d5;">
                @else
                    <a href="{{ asset('storage/'.$expense->bill_image) }}" target="_blank"
                        class="btn btn-sm btn-outline-success" style="border-radius:8px;">
                        <i class="ti-file mr-1"></i> View Bill
                    </a>
                @endif
            </div>
            @endif
            <div class="bill-upload-zone" onclick="document.getElementById('billFileInput').click()">
                ...
                <div class="buz-title">{{ $expense->bill_image ? 'Replace Bill' : 'Upload Bill' }}</div>
    ...
    <div class="ef-card">
        ...
        <div class="ef-card-body">
            <div style="font-size:12px;color:#8a94a6;line-height:1.9;">
                ...
                <div><i class="ti-check mr-1" style="color:#38a169;"></i> Approved expenses are locked for editing</div>
...
<div class="sticky-footer">
    ...
    <div style="display:flex;gap:10px;align-items:center;">
        ...
        <button type="submit" class="btn-save-exp" id="saveExpBtn">
            ...
            <span id="saveBtnText">Update Expense</span>
        </button>
    </div>
</div>
        ...
        <div class="modal-content" style="border:none;border-radius:12px;overflow:hidden;">
            ...
            <div class="modal-body" style="padding:20px;">
                <div class="form-group">
                    ...
                    <input type="text" id="newCatName" class="form-control ef-input" placeholder="e.g. Insurance, Cleaning..." maxlength="50">
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:8px;" id="iconPicker">
                        @foreach(['ti-dropbox'=>'Fuel','ti-map'=>'Map','ti-user'=>'Person','ti-cup'=>'Food','ti-settings'=>'Settings','ti-wrench'=>'Tools','ti-location-pin'=>'Location','ti-bag'=>'Bag','ti-home'=>'Home','ti-package'=>'Package','ti-credit-card'=>'Card','ti-more-alt'=>'Other'] as $ic => $il)
                        <div class="ico-sw" data-icon="{{ $ic }}" title="{{ $il }}"
                            style="width:42px;height:42px;border-radius:8px;background:#f4f6fb;color:#596579;cursor:pointer;display:flex;align-items:center;justify-content:center;border:2px solid transparent;transition:all .15s;">
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:8px;" id="colorPicker">
                        @foreach(['#e53e3e'=>'#fff5f5','#7c3aed'=>'#f5f3ff','#38a169'=>'#f0fff4','#d97706'=>'#fffbeb','#0369a1'=>'#f0f9ff','#b45309'=>'#fff8e1','#667eea'=>'#eef2ff','#8a94a6'=>'#f4f6fb'] as $clr => $clrbg)
                        <div class="clr-sw" data-color="{{ $clr }}" data-bg="{{ $clrbg }}"
                            style="width:38px;height:38px;border-radius:8px;background:{{ $clrbg }};border:2px solid {{ $clr }};cursor:pointer;transition:all .15s;">
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="modal-footer" style="border-top:1px solid #f0f2f7;padding:12px 20px;gap:8px;">
                ...
                <button type="button" id="saveCategoryBtn"
                    class="btn btn-sm" style="background:linear-gradient(135deg,#d97706,#b45309);color:#fff;border:none;border-radius:8px;font-weight:600;padding:7px 18px;">
...
<div class="modal fade" id="addTraderModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px;" role="document">
        <div class="modal-content" style="border:none;border-radius:12px;overflow:hidden;">
            ...
            <div class="modal-footer" style="border-top:1px solid #f0f2f7;padding:12px 20px;gap:8px;">
                <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal" style="border-radius:8px;">Cancel</button>
                <button type="button" id="saveQuickTraderBtn" class="btn btn-sm" style="background:linear-gradient(135deg,#d97706,#b45309);color:#fff;border:none;border-radius:8px;font-weight:600;padding:7px 18px;">
...
$(document).ready(function () {
    ...
    function formatCategoryOption(opt) {
        ...
        if (opt.id === '__add_new__') {
            return $('<span style="display:inline-flex;align-items:center;gap:8px;color:#d97706;font-weight:700;font-size:13px;">' +
                '<span style="width:28px;height:28px;border-radius:6px;background:#fffbeb;display:inline-flex;align-items:center;justify-content:center;"><i class="ti-plus"></i></span>' +
    ...
    if ($.fn.select2) {
        ...
        // Init all other selects except traderSelect and categorySelect
    ...
    $('#billFileInput').on('change', function () {
        ...
        if (['jpg','jpeg','png'].indexOf(ext) !== -1) {
            ...
            reader.onload = function (e) {
                $p.html('<img src="' + e.target.result + '" style="width:100%;border-radius:8px;border:2px solid #fed7aa;">');
            };
            reader.readAsDataURL(file);
        } else {
            $p.html('<div style="background:#f4f6fb;border-radius:8px;padding:10px;font-size:12px;display:flex;align-items:center;gap:8px;"><i class="ti-file" style="color:#d97706;font-size:18px;"></i><span style="font-weight:600;">' + file.name + '</span><span style="color:#8a94a6;">' + size + '</span></div>');
        }
    });
    ...
    $('#addNewCategoryBtn').on('click', function () {
        ...
        $('#newCatBg').val('');
        $('#newCatPreview').html('');
        $('#iconPicker .ico-sw').css({'border-color':'transparent','background':'#f4f6fb','color':'#596579'});
        $('#colorPicker .clr-sw').css('outline','');
        $('#newCatNameError,#newCatIconError,#newCatColorError').hide();
        $('#addCategoryModal').modal('show');
    });

    $('#iconPicker').on('click', '.ico-sw', function () {
        $('#iconPicker .ico-sw').css({'border-color':'transparent','background':'#f4f6fb','color':'#596579'});
        $(this).css({'border-color':'#d97706','background':'#fffbeb','color':'#d97706'});
        $('#newCatIcon').val($(this).data('icon'));
        updatePreview();
    });

    $('#colorPicker').on('click', '.clr-sw', function () {
        $('#colorPicker .clr-sw').css('outline','');
        $(this).css('outline','3px solid #1a2340');
        $('#newCatColor').val($(this).data('color'));
        $('#newCatBg').val($(this).data('bg'));
        updatePreview();
    });

    $('#newCatName').on('input', updatePreview);
    ...
    $('#saveCategoryBtn').on('click', function () {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { _token: '{{ csrf_token() }}', label: name, icon: icon, color: color, bg: bg },
            success: function (res) {
                if (res.success) {
                    // Add new option to the category dropdown (before "+ Add New Category")
                    var $newOpt = $('<option>', {
                        value: res.key,
                        text:  res.label,
                        'data-icon':  res.icon,
                    ...
                    if (typeof toastr !== 'undefined') {
                        ...
                        var $t = $('<div style="position:fixed;bottom:24px;right:24px;background:#1a2340;color:#fff;padding:12px 20px;border-radius:10px;font-size:13px;font-weight:600;box-shadow:0 4px 20px rgba(0,0,0,.2);z-index:9999;"><i class=\'ti-check mr-2\' style=\'color:#48bb78;\'></i>' + res.label + ' category added</div>');
                        $('body').append($t);
            ...
            error: function (xhr) {
                alert(xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to save.');
            },
            complete: function () {
    ...
    var oldTraderVal = '{{ old('trader_id', $expense->trader_id) }}';
    ...
    function rebuildTraderDropdown(catKey) {
        ...
        // No category selected → empty + disabled
        if (!catKey) {
            $('#noTradersMsg').hide();
    ...
    // Category dropdown — use select2:select/clear to avoid init-time fire
    ...
    $('#addTraderModal').on('hidden.bs.modal', function () {
        var $cat = $('#newTraderCategory');
        if ($.fn.select2 && $cat.hasClass('select2-hidden-accessible')) {
            $cat.select2('destroy');
        }
    });
    ...
    /* ── Add Accessory Row ──────────────────────────────────── */
    var accRowIdx = {{ ($expense->category === 'accessories') ? count($expense->accessories) : 1 }};
