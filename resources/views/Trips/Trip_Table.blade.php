...
<style>
    ...
    .inv-type-card.selected {
        border-color: #667eea;
        background: #eef2ff;
    }

    .inv-type-card .itc-icon {
        font-size: 26px;
        margin-bottom: 6px;
    }

    .inv-type-card .itc-title {
        font-weight: 800;
        font-size: 13px;
        color: #1a2340;
        margin-bottom: 2px;
    ...
    .inv-type-card .itc-tax {
        font-size: 11px;
        color: #667eea;
        font-weight: 700;
    }

    .inv-type-card .itc-desc {
        font-size: 10px;
        color: #8a94a6;
        margin-top: 3px;
    }
    ...
    tr.invoiced-row td:first-child {
            ...
            <div class="page-body">
                ...
                <div class="td-finance-strip">
                    <div class="td-fin-card">
                        ...
                        <div>
                            <div class="tfc-label">Total Freight</div>
                            <div class="tfc-value" style="color:#4338ca;">₹{{ number_format($stats['total_freight'],0) }}</div>
                        </div>
                    </div>
                    <div class="td-fin-card">
                        <div class="tfc-icon" style="background:#f0fff4;color:#38a169;"><i class="ti-check"></i></div>
                        <div>
                            <div class="tfc-label">Total Collected</div>
                            <div class="tfc-value" style="color:#38a169;">₹{{ number_format($stats['total_collected'],0) }}</div>
                        </div>
                    </div>
                    <div class="td-fin-card">
                        <div class="tfc-icon" style="background:#fff5f5;color:#e53e3e;"><i class="ti-alert"></i></div>
                        <div>
                            <div class="tfc-label">Outstanding</div>
                            <div class="tfc-value" style="color:#e53e3e;">₹{{ number_format($stats['total_outstanding'],0) }}</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="td-table-wrap">
                        <table class="table table-hover" id="tripsTable">
                            ...
                            <tbody>
                                ...
                                @empty
                                <tr>
                                    <td colspan="11" class="text-center py-5" style="color:#b0bac9;">
...
@push('scripts')
<script>
    // ── Invoice route URLs (kept short to avoid editor line-wrapping bugs) ──
    var urlInvGenerate = '{{ route('invoice.generate') }}';
    var urlInvExcel    = '{{ route('invoice.excel') }}';
    var urlInvPdf      = '{{ route('invoice.pdf') }}';
    ...
    $(document).ready(function() {
        ...
        $('#clearFilters').on('click', function() {
            ...
            $('.td-sf-pill[data-filter]').removeClass('active').css({
                'border-color': 'transparent',
                'background': '#f4f6fb'
            });
            $('.td-sf-pill[data-filter="all"]').addClass('active').css({
                'border-color': '#667eea',
                'background': '#eef2ff'
            });
            applyFilters();
        ...
        $('.td-sf-pill[data-filter]').on('click', function() {
            ...
            $(this).addClass('active').css({
                'border-color': pillColors[filter] || '#667eea',
                'background': pillBgs[filter] || '#eef2ff'
            });
        ...
        $('#invoiceTypeModal').on('show.bs.modal', function() {
            ...
            /* ── Auto-detect invoice type based on party GST registration ── */
            var hasGst = 0,
                noGst = 0;
            $('.trip-check:checked').each(function() {
                var $row = $(this).closest('tr.trip-row');
                if ($row.attr('data-party-has-gst') == '1') {
                    hasGst++;
                } else {
                    noGst++;
                }
            });
            /* Default to Normal if any party has GST or no trips selected, else RCM */
            var suggested = (hasGst > 0 || (hasGst === 0 && noGst === 0)) ? 'normal' : 'rcm';
            $('#selectedInvType').val(suggested);
            $('#selectedInvTypeLabel').val(invTypeLabels[suggested] || 'TAX INVOICE');
            $('.inv-type-card').removeClass('selected');
            $('.inv-type-card[data-type="' + suggested + '"]').addClass('selected');
            $('#invTypeNote').show();
            $('#invTypeNoteText').text(invTypeNotes[suggested] || '');
        });
        ...
        $('#btnExcelDownload').on('click', function() {
            ...
            var type = $('#selectedInvType').val() || 'normal';
            var typeLabel = $('#selectedInvTypeLabel').val() || 'TAX INVOICE';
            var form = $('<form method="POST" action="' + urlInvExcel + '"></form>');
            form.append('<input type="hidden" name="_token" value="{{ csrf_token() }}">');
            form.append('<input type="hidden" name="invoice_type" value="' + type + '">');
            form.append('<input type="hidden" name="invoice_type_label" value="' + typeLabel + '">');
            ids.forEach(function(id) {
        ...
        $('#btnPdfDownload').on('click', function() {
            ...
            var type = $('#selectedInvType').val() || 'normal';
            var typeLabel = $('#selectedInvTypeLabel').val() || 'TAX INVOICE';
            var form = $('<form method="POST" action="' + urlInvPdf + '" target="_blank"></form>');
            form.append('<input type="hidden" name="_token" value="{{ csrf_token() }}">');
            form.append('<input type="hidden" name="invoice_type" value="' + type + '">');
            form.append('<input type="hidden" name="invoice_type_label" value="' + typeLabel + '">');
            ids.forEach(function(id) {
