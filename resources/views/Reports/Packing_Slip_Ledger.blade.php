...
<div class="rpt-header">
    <div class="row align-items-center">
        <div class="col-md-8" style="position:relative;z-index:1;">
            ...
            <div class="sub">{{ $totalTrips }} trips &bull; Total Qty: {{ number_format($totalQty,0) }}</div>
        </div>
...
<form method="GET" action="{{ route('reports.packing-slip-ledger') }}">
<div class="rpt-filter">
    ...
    <a href="{{ route('reports.packing-slip-ledger') }}" class="btn btn-outline-secondary btn-sm" style="border-radius:8px;padding:8px 14px;">
...
<div class="col-summary">
    <div class="col-sum-card">
        ...
        <div class="csc-value" style="color:#9333ea;">{{ $totalTrips }}</div>
    </div>
    <div class="col-sum-card">
        <div class="csc-label">Total Quantity</div>
        <div class="csc-value" style="color:#16a34a;">{{ number_format($totalQty,0) }}</div>
    </div>
    <div class="col-sum-card">
        <div class="csc-label">Parties</div>
        <div class="csc-value" style="color:#2563eb;">{{ $trips->pluck('party_id')->unique()->count() }}</div>
    </div>
</div>

<div class="rpt-card">
    <div class="rpt-card-header">
        <h6><i class="ti-layout mr-2" style="color:#9333ea;"></i>Packing Slips ({{ $totalTrips }} records)</h6>
        ...
        <table class="table table-striped table-bordered" id="psTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:11px;">
                        ...
                        <div style="font-size:9px;color:#8a94a6;">{{ $t->from_state ?? '' }} → {{ $t->to_state ?? '' }}</div>
                        @endif
                    </td>
                    <td>{{ $t->material ?? '—' }}</td>
                    ...
                    <td>
                        @if($t->invoice_no)
                        <span class="rpt-badge" style="background:#eef2ff;color:#4338ca;">{{ $t->invoice_no }}</span>
                        @else
            ...
            <tfoot>
                <tr style="background:#f8fafc;font-weight:800;">
                    ...
                    <td></td>
                    <td style="text-align:right;color:#16a34a;">{{ number_format($totalQty,0) }}</td>
                    <td colspan="2"></td>
                </tr>
            </tfoot>
            @endif
...
function exportPsExcel() {
    ...
    a.href   = url;
    a.download = 'Packing_Slip_Ledger_{{ now()->format("Y-m-d") }}.xls';
