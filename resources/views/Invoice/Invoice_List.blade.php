            ...
            <tbody>
                ...
                <tr class="inv-row"
                    ...
                    <td style="text-align:center;white-space:nowrap;" onclick="event.stopPropagation();">
                        ...
                        @endif
                        {{-- Download PDF --}}
                        <a href="{{ route('invoice.download.pdf', $inv->invoice_no) }}"
                            class="btn btn-sm"
                            style="background:#eff6ff;color:#3b82f6;border:1px solid #bfdbfe;border-radius:7px;padding:4px 10px;font-size:12px;font-weight:600;margin-left:4px;"
                            title="Download Invoice PDF">
                            <i class="ti-download"></i>
                        </a>
                        {{-- Send PDF via WhatsApp --}}
                        <button type="button"
                            class="btn btn-sm btn-wa-invoice"
                            style="background:#f0fdf4;color:#25d366;border:1px solid #86efac;border-radius:7px;padding:4px 10px;font-size:12px;font-weight:600;margin-left:4px;"
                            title="Send Invoice PDF to customer via WhatsApp"
                            onclick="sendInvoiceWhatsApp('{{ $inv->invoice_no }}', this)">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="#25d366" style="display:inline-block;vertical-align:middle;margin-top:-2px;">
...
// ── WhatsApp: send invoice PDF to customer (no browser download) ─────────────
function sendInvoiceWhatsApp(invoiceNo, btn) {
    if (!confirm('Send Invoice ' + invoiceNo + ' PDF to the customer via WhatsApp?')) return;

    var $btn = $(btn);
    $btn.prop('disabled', true).html('<i class="ti-reload"></i>');

    $.ajax({
        url:  '/invoice/' + invoiceNo + '/whatsapp',
        type: 'POST',
        data: { _token: $('meta[name="csrf-token"]').attr('content') },
        success: function(res) {
            if (res.success) {
                $btn.html('<i class="ti-check"></i>')
                    .css({'background':'#dcfce7','color':'#16a34a','border-color':'#86efac'});
                if (typeof toastr !== 'undefined') {
                    toastr.success(res.message, 'Sent via WhatsApp');
                }
                setTimeout(function() {
                    $btn.prop('disabled', false)
                        .html('<i class="ti-comment-alt"></i>')
                        .css({'background':'#f0fdf4','color':'#25d366','border-color':'#86efac'});
                }, 3000);
            } else {
                $btn.prop('disabled', false).html('<i class="ti-comment-alt"></i>');
                if (typeof toastr !== 'undefined') {
                    toastr.error(res.message, 'WhatsApp Failed');
                } else {
                    alert(res.message);
                }
            }
        ...
        error: function(xhr) {
            $btn.prop('disabled', false).html('<i class="ti-comment-alt"></i>');
            var msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'Failed to send.';
            if (typeof toastr !== 'undefined') {
