...
<style>
    ...
    .veh-stat.stat-total .sc-icon {
        background: #eef2ff;
        color: #667eea;
    }

    .veh-stat.stat-total .sc-value {
        color: #667eea;
    }

    .veh-stat.stat-active {
        border-left-color: #38a169;
    }

    .veh-stat.stat-active .sc-icon {
        background: #f0fff4;
        color: #38a169;
    }

    .veh-stat.stat-active .sc-value {
        color: #38a169;
    }

    .veh-stat.stat-expiry {
        border-left-color: #e53e3e;
    }

    .veh-stat.stat-expiry .sc-icon {
        background: #fff5f5;
        color: #e53e3e;
    }

    .veh-stat.stat-expiry .sc-value {
        color: #e53e3e;
    }
    ...
    .veh-table-card .card-header-custom h5 {
        margin: 0;
        font-size: 15px;
        font-weight: 700;
        color: #1a2340;
    ...
    .veh-table-card .card-header-custom .sub {
        font-size: 12px;
        color: #8a94a6;
        margin-top: 2px;
    }
    ...
    .veh-table tbody tr:last-child td {
        border-bottom: none;
    }
    ...
    .veh-panel-header .panel-close:hover {
        background: #e53e3e;
    }
    ...
    .form-group-veh .select2-container {
        width: 100% !important;
    }
    ...
    .btn-panel-save {
        background: linear-gradient(135deg, #667eea 0%, #1a2340 100%);
        color: #fff;
        border: none;
        border-radius: 8px;
        padding: 9px 22px;
        ...
        <div class="page-wrapper">
            <div class="page-body">
                ...
                <div class="veh-header">
                    <div class="d-flex align-items-center justify-content-between flex-wrap" style="gap:12px;">
                        ...
                        <div class="header-actions">
                            <a href="{{ route('vehicle.add') }}" class="btn-add-veh"
                                onclick="event.preventDefault(); softNav('{{ route('vehicle.add') }}');">
                    ...
                    <div class="col-sm-4 mb-3 mb-sm-0">
                        <div class="veh-stat stat-total">
                            ...
                            <div>
                                ...
                                <div class="sc-value">{{ $totalVehicles }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4 mb-3 mb-sm-0">
                        <div class="veh-stat stat-active">
                            ...
                            <div>
                                ...
                                <div class="sc-value">{{ $activeVehicles }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4">
                ...
                <div class="veh-filter-bar">
                    ...
                    <select id="filterOwnerType" class="form-control" style="min-width:90px;max-width90px;border-radius:8px;border-color:#e2e8f0;font-size:13px;min-height:40px;">
                ...
                <div class="veh-table-card">
                    <div class="card-header-custom">
                        <div>
                            <h5>
                                ...
                                <span id="vehCountBadge" style="background:#eef2ff;color:#667eea;font-size:11px;font-weight:700;padding:2px 8px;border-radius:12px;margin-left:6px;">{{ $totalVehicles }}</span>
                            </h5>
                            <div class="sub">All registered vehicles are shown below</div>
                        </div>
                        <a href="{{ route('vehicle.add') }}" class="btn btn-primary btn-sm" style="border-radius:8px;"
                            onclick="event.preventDefault(); softNav('{{ route('vehicle.add') }}');">
                                    ...
                                    <td style="text-align:center;">
                                        <div style="display:flex;align-items:center;justify-content:center;gap:5px;">
                                            ...
                                            <a href="{{ route('vehicle.edit', $vehicle->id) }}" class="btn-action-edit" title="Edit"
                                                onclick="event.preventDefault(); softNav('{{ route('vehicle.edit', $vehicle->id) }}');">
                                                <i class="ti-pencil"></i>
                                            </a>
                    ...
                    <div class="table-pagination-wrap">
                        <div class="info">
                            Showing <strong>{{ $vehicles->firstItem() ?? 0 }}</strong>–<strong>{{ $vehicles->lastItem() ?? 0 }}</strong> of <strong>{{ $totalVehicles }}</strong>
                        </div>
                        <div>
...
<script>
    ...
    function openViewPanel(id) {
        ...
        $.getJSON('{{ url("/vehicle/data") }}/' + id, function(v) {
            ...
            var editBtn = document.getElementById('viewPanelEditBtn');
