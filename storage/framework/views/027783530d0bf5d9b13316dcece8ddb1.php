...
.frm-card { background:#fff; border-radius:12px; box-shadow:0 2px 10px rgba(0,0,0,.06); overflow:hidden; }
.frm-card-body { padding:24px; }
.frm-card-body .control-label { font-size:12px; font-weight:700; color:#374151; margin-bottom:4px; }
.frm-card-body .form-control { border-radius:8px; border-color:#e2e8f0; min-height:42px; font-size:13px; }
.frm-card-body .form-control:focus { border-color:#9333ea; box-shadow:0 0 0 2px rgba(147,51,234,.15); }
.entry-section { background:#fafbff; border:1px solid #e8ecf4; border-radius:10px; overflow:hidden; transition:box-shadow .3s, border-color .3s; }
.entry-section.highlight-new { box-shadow:0 0 0 3px #9333ea, 0 0 20px rgba(147,51,234,.25); border-color:#9333ea; }
.entry-section-header { background:linear-gradient(135deg,#9333ea 0%,#7c3aed 100%); color:#fff; padding:6px 10px; font-size:11px; font-weight:900; text-align:center; }
.entry-table { width:100%; border-collapse:collapse; }
.entry-table th { background:#f0f2f7; color:#14213d; font-weight:800; font-size:10px; text-transform:uppercase; letter-spacing:.3px; padding:4px 6px; border-bottom:2px solid #dde1ea; text-align:center; }
.entry-table td { padding:2px; border-bottom:1px solid #f0f2f7; }
.entry-table .sno-cell { text-align:center; font-weight:700; color:#374151; font-size:11px; width:30px; }
.entry-table input[type="number"] { width:90%; border:1px solid #e2e8f0; border-radius:4px; padding:1px 2px; font-size:10px; min-height:18px; text-align:center; outline:none; transition:border-color .15s; -moz-appearance:textfield; }
.entry-table input[type="number"]::-webkit-outer-spin-button,
.entry-table input[type="number"]::-webkit-inner-spin-button { -webkit-appearance:none; margin:0; }
.entry-table input[type="number"]:focus { border-color:#9333ea; box-shadow:0 0 0 2px rgba(147,51,234,.12); }
.total-row td { font-weight:900; font-size:11px; background:#f8fafc; border-top:2px solid #9333ea; padding:4px 6px; }
.total-row .total-val { color:#9333ea; text-align:right; padding-right:6px; font-weight:900; }
.entry-section-footer { padding:4px 10px; background:#f8fafc; border-top:1px solid #e8ecf4; font-size:12px; color:#6b7280; }
.bale-grid { display:grid; grid-template-columns:repeat(5,1fr); gap:12px; margin-bottom:16px; }
</style>
...
<div style="padding:24px;">
    ...
    <div class="row align-items-center">
        <div class="col-md-6" style="position:relative;z-index:1;">
            <h4><i class="ti-plus mr-2"></i><?php echo e(isset($slip) ? 'Edit' : 'Add'); ?> Packing Slip</h4>
            <div class="sub"><?php echo e(isset($slip) ? 'Update' : 'Create a new'); ?> packing slip with bale entry</div>
        </div>
        <div class="col-md-6 text-right" style="position:relative;z-index:1;">
            <a href="<?php echo e(route('packing-slip.index')); ?>" class="btn btn-sm" style="border-radius:8px;background:rgba(255,255,255,.2);color:#fff;border:1px solid rgba(255,255,255,.3);padding:7px 16px;font-weight:600;">
...
<div class="frm-card">
    <div class="frm-card-body">
        <form method="POST" action="<?php echo e(route('packing-slip.store')); ?>" id="slipForm" onsubmit="return false;">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="id" id="slipId" value="<?php echo e(isset($slip) ? $slip->id : ''); ?>">

            <div class="row" style="margin-bottom:4px;">
                <div class="col-md-3 form-group">
                    <label class="control-label">Bill No</label>
                    <input type="text" name="bill_no" class="form-control" placeholder="Bill number" value="<?php echo e(isset($slip) ? $slip->bill_no : ''); ?>">
                </div>
                <div class="col-md-3 form-group">
                    <label class="control-label">Lot No</label>
                    <input type="text" name="lot_no" class="form-control" placeholder="Lot number" value="<?php echo e(isset($slip) ? $slip->lot_no : ''); ?>">
                </div>
                <div class="col-md-3 form-group">
                    <label class="control-label">Date <span style="color:#dc2626;">*</span></label>
                    <input type="date" name="slip_date" class="form-control" required value="<?php echo e(isset($slip) ? $slip->slip_date?->format('Y-m-d') : date('Y-m-d')); ?>">
                </div>
                <div class="col-md-3 form-group">
                    ...
                    <select name="pack_customer_id" class="form-control select2" required>
                        ...
                        <option value="<?php echo e($c->id); ?>" <?php echo e(isset($slip) && $slip->pack_customer_id == $c->id ? 'selected' : ''); ?>><?php echo e($c->name); ?></option>
            ...
            <div class="row" style="margin-bottom:4px;">
                <div class="col-md-3 form-group">
                    ...
                    <select name="quality" class="form-control select2">
                        ...
                        <option value="<?php echo e($q->name); ?>" <?php echo e(isset($slip) && $slip->quality == $q->name ? 'selected' : ''); ?>><?php echo e($q->name); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div class="col-md-3 form-group">
...
function createSection(baleNo) {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() { sec.classList.remove('highlight-new'); }, 1200);
...
function calcTotals() {
    ...
    var allMeters = 0, firstBale = null, lastBale = null;
    sections.forEach(function(sec) {
        var meters = sec.querySelectorAll('.meter-input');
        var mTotal = 0, wTotal = 0;
        meters.forEach(function(inp) { mTotal += parseFloat(inp.value) || 0; });
        sec.querySelectorAll('.weight-input').forEach(function(inp) { wTotal += parseFloat(inp.value) || 0; });
...
function ajaxSave(callback) {
    ...
    if (!form) { console.log('slipForm not found'); return; }
    ...
    itemKeys.forEach(function(k) { formData.delete(k); });
    ...
    fetch('<?php echo e(route("packing-slip.store")); ?>', {
        method: 'POST',
        body: formData,
        headers: {
...
var editBaleItems = <?php echo isset($slip) ? json_encode($slip->baleItems) : 'null'; ?>;
var initialBaleNo = <?php echo e($nextBaleNo ?? 1); ?>;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editBaleItems && editBaleItems.length) {
        ...
        var baleNos = Object.keys(groups).map(Number).sort(function(a, b) { return a - b; });
        baleNos.forEach(function(bn) {
            ...
            groups[bn].forEach(function(item) {
                ...
                if (row) {
                    ...
                    var weightInput = row.querySelector('.weight-input');
                    if (meterInput) { meterInput.value = item.meter; formatMeter(meterInput); }
    ...
    document.getElementById('closeBtn').addEventListener('click', function() {
        if (saved || document.getElementById('slipId').value) {
            ...
            setTimeout(function() { window.location.href = '<?php echo e(route("packing-slip.index")); ?>'; }, 500);
        } else {
            window.location.href = '<?php echo e(route("packing-slip.index")); ?>';
        }
    });
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Control') {
        ...
        setTimeout(function() { ctrlPressed = false; }, 500);
    } else if (ctrlPressed) {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.fullscreen', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\Kovai-Trans\resources\views/PackingSlip/create.blade.php ENDPATH**/ ?>